document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('imageUpload').addEventListener('change', function (e) {
    ...
    reader.onload = function (event) {
      ...
      imgObj.onload = function () {
        ...
        document.getElementById('uploadImageContainer').classList.add('d-none');
        document.getElementById('editImageContainer').classList.remove('d-none');
      };
    };
      ...
      .then(blob => {
        ...
        navigator.clipboard.write([item]).then(
          () => {
            ...
            setTimeout(() => {
                button.querySelector("#copyImageClip").innerText = "Copy to Clipboard";
            }, 2000);
          },
          (error) => {
            document.querySelector("#copyImageClip").innerText = "Failed to Copy Image!";
            setTimeout(() => {
              document.querySelector("#copyImageClip").innerText = "Copy to Clipboard";
            }, 2000);
  ...
  function createTrimmedCanvas(originalCanvas, callback) {
    ...
    originalCanvas.clone(function(clonedCanvas) {
      ...
      let boundingRect = { left: Infinity, top: Infinity, right: -Infinity, bottom: -Infinity };
      ...
      if (clonedCanvas.backgroundImage) {
        ...
        boundingRect.right = Math.max(boundingRect.right, bgBounding.left + bgBounding.width);
        boundingRect.bottom = Math.max(boundingRect.bottom, bgBounding.top + bgBounding.height);
      }
      ...
      objects.forEach(obj => {
        ...
        boundingRect.right = Math.max(boundingRect.right, objBounding.left + objBounding.width);
        boundingRect.bottom = Math.max(boundingRect.bottom, objBounding.top + objBounding.height);
      });
      ...
      if (clonedCanvas.backgroundImage) {
        ...
        trimmedCanvas.setBackgroundImage(bgImage, trimmedCanvas.renderAll.bind(trimmedCanvas), {
      ...
      const EXPORT_MULTIPLIER = currentScaleFactor > 0 ? 1 / currentScaleFactor : 1;
